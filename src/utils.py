...
def get_lines_from_page(page):
    ...
    for block in page_dict['blocks']:
        ...
        if block['bbox'][0] < mid + 20 and block['bbox'][2] < mid + 20:
            left_blocks.append(block)
        elif block['bbox'][0] > mid - 20 and block['bbox'][2] > mid - 20:
            right_blocks.append(block)
        elif not re.search('Copyright', content):
            topic = re.sub(r'\s*Posters:?\s*|\s*\d+\s*', '', content).strip()
    left_lines = get_lines_from_column(left_blocks)
    right_lines = get_lines_from_column(right_blocks)
...
def get_fonts_and_texts(line):
    return [span['font'] for span in line['spans']], [span['text'] for span in line['spans']] if line else [], []
